...
shared:	$(COMMON_OBJECTS)
	$(CXX) $+ $(SHARED_OPTIONS) $(LDFLAGS) -o $(TUIO_SHARED)
	
dump:	$(COMMON_OBJECTS) $(DUMP_OBJECTS)
	$(CXX) -o $(TUIO_DUMP) $+ $(LDFLAGS)

demo:	$(COMMON_OBJECTS) $(DEMO_OBJECTS)
	$(CXX) -o $(TUIO_DEMO) $+ $(SDL_LDFLAGS) $(LDFLAGS) $(FRAMEWORKS)

simulator:	$(COMMON_OBJECTS) $(SIMULATOR_OBJECTS)
	$(CXX) -o $(SIMPLE_SIMULATOR) $+ $(SDL_LDFLAGS) $(LDFLAGS) $(FRAMEWORKS)
